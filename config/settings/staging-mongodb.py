...
DATABASES = {
    'default': {
        'ENGINE': 'djongo',  # Bukan postgresql!
        'NAME': os.getenv('STAGGING_DB_NAME'),
        'CLIENT': {
            'host': f"mongodb://{os.getenv('STAGGING_DB_USER')}:{os.getenv('STAGGING_DB_PASSWORD')}@{os.getenv('STAGGING_DB_HOST')}:{os.getenv('STAGGING_DB_PORT')}/{os.getenv('STAGGING_DB_NAME')}"
        }
    }
}
